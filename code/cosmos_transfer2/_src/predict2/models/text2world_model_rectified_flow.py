...
class Text2WorldModelRectifiedFlow(ImaginaireModel):
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        steps_count=0
        reuse_cache=0#note the times of reusing cache
        cache_threshold=8
        result=[]
        elur_result=[]

        mask_bool=None
        latent_model_input = torch.randn(1, 16, 12, 88, 120)
        enter_flatten_flag=False
        for _, t in enumerate(timesteps_iter):
            ...
            if enter_flatten_flag==False and steps_count<32 :#when not enter the flatten area
                ...
                reuse_cache=0
            elif enter_flatten_flag==True and steps_count<32:#
                if reuse_cache<cache_threshold:
                    velocity_pred=last_velocity_pred
                    reuse_cache+=1
                    temp_x0 = self.sample_scheduler.step(
                    velocity_pred.unsqueeze(0), t, latents[0].unsqueeze(0), return_dict=False, generator=seed_g
                    )[0]                      
            ...
            if steps_count>=1:
                #print(f"euclidean_distance_pred:{euclidean_distance(last_velocity_pred,velocity_pred)}")
                elur_result.append(euclidean_distance(last_velocity_pred,velocity_pred))  
            if len(elur_result)>=2 and steps_count>=2:
                elur_index=steps_count-1
                drop_distance=elur_result[elur_index-1]-elur_result[elur_index]
                begin_drop_distance=elur_result[0]-elur_result[1]
                if enter_flatten_flag==False:
                    if drop_distance<=0.2*begin_drop_distance and drop_distance>0 and steps_count<6:
                        enter_flatten_flag=True 
                    elif steps_count>=6:
                        enter_flatten_flag=True
            latents = temp_x0.squeeze(0)
            last_velocity_pred=velocity_pred
            steps_count+=1
