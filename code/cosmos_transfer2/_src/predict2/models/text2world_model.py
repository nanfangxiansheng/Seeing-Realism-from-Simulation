...
class DiffusionModel(ImaginaireModel):
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        if self.config.use_flowunipc_scheduler:#没有使用flowunipc这个调度器
            sample_scheduler = FlowUniPCMultistepScheduler(
                num_train_timesteps=1000, shift=1, use_dynamic_shifting=False
            )
            noise = misc.arch_invariant_rand(#进行噪声初始化，arch_invariant_rand生成和硬件架构无关的随机数
            ...
            with torch.no_grad():#舍弃梯度降低GPU压力
                x0_fn = self.get_x0_fn_from_batch(data_batch, guidance, is_negative_prompt=is_negative_prompt)
                latents = noise
                ...
                if INTERNAL:
                    ...
                    timesteps_iter = tqdm.tqdm(timesteps, desc="Generating samples", total=len(timesteps))#这里就是generating samples的进度条的来源
                for _, t in enumerate(timesteps_iter):#timesteps_iter是一个时间步序列
                    latent_model_input = latents#输入的潜在表示，也就是噪声
                    timestep = [t]

                    # our model supports 0-1 while the t is 0-1000
                    timestep = torch.stack(timestep) / 1000
                    noise_pred = x0_fn(latent_model_input, timestep.unsqueeze(0))#调用预测函数
                    temp_x0 = sample_scheduler.step(#前向传播一次，调度器步进
