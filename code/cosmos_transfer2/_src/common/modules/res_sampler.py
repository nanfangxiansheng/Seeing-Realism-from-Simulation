...
class Sampler(torch.nn.Module):#采样器定义
    ...
    def forward(#最关键的前向传播
        ...
        timestamps_cfg = SolverTimestampConfig(nfe=num_steps, t_min=sigma_min, t_max=sigma_max, order=rho)#加载时间戳配置
        sampler_cfg = SamplerConfig(solver=solver_cfg, timestamps=timestamps_cfg, sample_clean=True)#加载配置文件
    ...
    def _forward_impl(
        ...
        denoised_output = differential_equation_solver(#扩散模型对输入噪声进行去噪
...
def differential_equation_solver(
    ...
    def sample_fn(input_xT_B_StateShape: torch.Tensor) -> torch.Tensor:
        ...
        x_at_eps, _ = fori_loop(0, num_step, step_fn, [input_xT_B_StateShape, None])#在fori_loop中进行step_fn的循环调用
        return x_at_eps
